...
export const Login: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!auth) {
            setError("Error: Autenticación no disponible (Falta API Key)");
            return;
        }
        try {
    ...
    return (
        <div className="pt-24 min-h-screen bg-slate-50 dark:bg-background-dark flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-100 dark:border-slate-700">
                <h2 className="text-3xl font-bold text-center mb-8 text-slate-800 dark:text-white">Iniciar Sesión</h2>
                ...
                <button
                    ...
                    className="w-full bg-white dark:bg-slate-700 text-slate-700 dark:text-white font-semibold py-3 px-4 rounded-xl border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 transition-all flex items-center justify-center gap-3 mb-6"
                >
                ...
                <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center text-sm">
                        <span className="px-2 bg-white dark:bg-slate-800 text-slate-500">O con email</span>
                    </div>
                </div>
                ...
                <div className="mt-6 text-center text-slate-600 dark:text-slate-400">
                    <p>¿No tienes cuenta? <Link to="/registro" className="text-primary font-bold hover:underline">Regístrate aquí</Link></p>
                </div>
            </div>
        </div>
    );
