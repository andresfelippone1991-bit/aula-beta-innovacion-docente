...
export const ResourceRequestModal: React.FC<ResourceRequestModalProps> = ({ isOpen, onClose }) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!finalEmail) {
            setStatus('error');
            setErrorMsg('Por favor, ingresa un correo electrónico de contacto.');
            return;
        }
        ...
        try {
            ...
            const response = await fetch("https://formsubmit.co/ajax/aula.beta1@gmail.com", {
                ...
                body: JSON.stringify({
                    _subject: "Nuevo Recurso Solicitado - Aula Beta",
                    _captcha: "false",
                    "Email del Solicitante": finalEmail,
                    "Detalle de la Solicitud": requestText,
            ...
            if (response.ok) {
                ...
                throw new Error("No se pudo enviar la solicitud por correo. Inténtalo de nuevo.");
            }
        } catch (error: any) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            ...
            >
                {status === 'success' ? (
                    <div className="p-8 text-center flex flex-col items-center justify-center min-h-[320px] animate-in zoom-in duration-300">
                        ...
                        <p className="text-slate-600 dark:text-slate-400 max-w-xs mx-auto">
                            Tu solicitud se ha enviado con éxito. Diseñaremos el recurso para la comunidad y te contactaremos.
                        </p>
                    </div>
                ) : (
                    <div className="p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {user?.email ? (
                                <div className="p-3 bg-blue-50/50 dark:bg-blue-950/20 border border-blue-100/50 dark:border-blue-900/30 rounded-xl text-xs text-slate-600 dark:text-slate-400 flex items-center gap-2">
                                    <span className="material-icons-round text-blue-500 text-sm">account_circle</span>
                                    <span>Solicitando como: <strong className="text-blue-600 dark:text-blue-400 font-semibold">{user.email}</strong></span>
                                </div>
                            ) : (
                                <div>
                            ...
                            <div>
                                <label htmlFor="requestText" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                                    ¿Qué recurso necesitas?
                                </label>
                                <textarea
                                    id="requestText"
                                    className="w-full h-32 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none resize-none transition-all placeholder-slate-400 dark:placeholder-slate-600 text-sm"
                            ...
                            <div className="flex gap-3 justify-end pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={status === 'submitting'}
                                    className="px-6 py-2.5 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20 active:scale-95 disabled:opacity-70 disabled:hover:bg-blue-600 flex items-center justify-center gap-2 text-sm min-w-[140px]"
                                >
                                    {status === 'submitting' ? (
                                        <>
