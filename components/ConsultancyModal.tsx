    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch("https://formsubmit.co/ajax/aula.beta1@gmail.com", {
                ...
                body: JSON.stringify({
                    _subject: "Nueva Consultoría Agendada - Aula Beta",
                    _captcha: "false",
                    _replyto: email,
                    "Email de Contacto": email,
                    "Motivo de la Consultoría": reason,
            ...
            if (response.ok) {
                ...
                throw new Error("No se pudo agendar la consultoría por correo. Inténtalo de nuevo.");
            }
        } catch (error: any) {
            console.error('Error submitting consultancy:', error);
            setStatus('error');
            setErrorMsg(error.message || 'Ocurrió un error al agendar. Inténtalo más tarde.');
        }
    };
                ...
                {status === 'success' ? (
                    <div className="p-8 text-center flex flex-col items-center justify-center min-h-[360px] animate-in zoom-in duration-300">
                        ...
                        <p className="text-slate-600 dark:text-slate-400 max-w-xs mx-auto">
                            Tus datos han sido registrados. Nos pondremos en contacto contigo a la brevedad para coordinar la reunión.
                        </p>
                    </div>
                ) : (
                    <div className="p-6">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label htmlFor="reason" className="block text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1.5">
                            ...
                            <div className="flex gap-3 justify-end pt-2">
                                ...
                                <button
                                    type="submit"
                                    disabled={status === 'submitting'}
                                    className="px-6 py-2.5 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20 active:scale-95 disabled:opacity-70 disabled:hover:bg-blue-600 flex items-center justify-center gap-2 text-sm min-w-[120px]"
                                >
                                    {status === 'submitting' ? (
                                        <>
